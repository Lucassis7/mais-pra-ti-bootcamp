// 3. Faça um algoritmo que pergunte a distância que um passageiro deseja percorrer em Km. Calcule o preço da passagem, cobrando R$ 0.50 por Km para viagens até 200 Km e R$ 0.45 para viagens mais longas.

const prompt = require("prompt-sync")();

const travelDistance = parseInt(prompt("Qual a distância da viagem? "));

if (travelDistance <= 200) {
  const travelCost = travelDistance * 0.5;

  console.log(`O custo da viagem ficou por: R$ ${travelCost.toFixed(2)}`);
} else {
  const travelCost = travelDistance * 0.45;

  console.log(`O custo da viagem ficou por: R$ ${travelCost.toFixed(2)}`);
}
